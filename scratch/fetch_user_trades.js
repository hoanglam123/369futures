...
async function main() {
  ...
  const symbols = ['TAUSDT', 'BOMEUSDT', 'SKYAIUSDT', 'BANANAS31USDT', 'TRUSTUSDT', 'STEEMUSDT', 'BANKUSDT', 'BICOUSDT', 'KAITOUSDT', 'HOLOUSDT', 'LUMIAUSDT', 'BTRUSDT', 'ONDOUSDT', 'HYPEUSDT', 'PIEVERSEUSDT'];

  for (const sym of symbols) {
    try {
      const trades = await getUserTrades(sym, startYesterday, endNow);
      if (trades.length > 0) {
        console.log(`\n=== [${sym}] Trades (${trades.length} fills) ===`);
        for (const t of trades) {
          const timeStr = new Date(t.time).toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' });
          console.log(`   ${timeStr} | ${t.side.padEnd(4)} | Qty: ${t.qty} @ $${t.price} | PnL: ${parseFloat(t.realizedPnl) >= 0 ? '+' : ''}${parseFloat(t.realizedPnl).toFixed(4)} USDT | Fee: ${t.commission} ${t.commissionAsset}`);
        }
      }
    } catch (e) {
