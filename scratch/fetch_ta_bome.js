...
async function main() {
  ...
  for (const sym of ['TAUSDT', 'BOMEUSDT']) {
    try {
      ...
      for (const t of trades) {
        ...
        console.log(`   ${timeStr} | ${t.side.padEnd(4)} | Qty: ${t.qty} @ $${t.price} | PnL: ${parseFloat(t.realizedPnl) >= 0 ? '+' : ''}${parseFloat(t.realizedPnl).toFixed(4)} USDT`);
      }
    } catch (e) {
